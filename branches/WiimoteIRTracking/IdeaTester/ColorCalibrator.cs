...
{
    public partial class ColorCalibrator : Form
    {
        ...
        bool _whiteBalancingOn = false;

        ColorCalibrationState[] _colorCalibrationInformaiton;

        public ColorCalibrator(ColorCalibrationState[] colorStates)
        {
            InitializeComponent();
            // Default learning
            ...
            _colorCalibrationInformaiton = colorStates;
        }
        ...
        {
            if (bc == BandColor.White)
            {
                Xyz WP = new Xyz(95.05, 100.00, 108.88);
                AdjWP(bandImage.Convert<Bgr,Byte>(), WP, AdjXYZ);
                _whiteBalancingOn = true;
                return;
            }
        ...
        private void performOperation()
        {
            ...
            if (_whiteBalancingOn)
            {
                Image<Xyz, byte> XYZimg = new Image<Xyz, byte>(source.Width, source.Height);
                CvInvoke.cvCvtColor(source, XYZimg, COLOR_CONVERSION.CV_BGR2XYZ);

                Image<Gray, byte> Ximg = new Image<Gray, byte>(source.Width, source.Height);
                Image<Gray, byte> Yimg = new Image<Gray, byte>(source.Width, source.Height);
                Image<Gray, byte> Zimg = new Image<Gray, byte>(source.Width, source.Height);
                //Image<Xyz, byte> unk = new Image<Xyz, byte>(sampleImg.Width, sampleImg.Height);
                CvInvoke.cvSplit(XYZimg, Ximg, Yimg, Zimg, IntPtr.Zero);
                Ximg._Mul(AdjXYZ[0]);
                Yimg._Mul(AdjXYZ[1]);
                Zimg._Mul(AdjXYZ[2]);
                Image<Xyz, byte> XYZimg2 = new Image<Xyz, byte>(source.Width, source.Height);
                CvInvoke.cvMerge(Ximg, Yimg, Zimg, IntPtr.Zero, XYZimg2);

                Image<Bgr, byte> wpImg = new Image<Bgr, byte>(source.Width, source.Height);
        ...
        private void btnPickBandRegion_Click(object sender, EventArgs e)
        {
            ...
            if (_waitingToLearnBand == BandColor.Red) lblStatus.Text = "Select the Red area in the source";
        }
            ...
            if (_statusWaitingForBandRegionSelection && _statusDragStarted)
            {
                ...
                switch (_waitingToLearnBand)
                {
                    case BandColor.White:
                        lblStatus.Text = "Select the Red band in the source";
        ...
        private void cbxWhiteBalance_CheckedChanged(object sender, EventArgs e)
        {
            _whiteBalancingOn = cbxWhiteBalance.Checked;
        }

        private void btnLearnWhite_Click(object sender, EventArgs e)
        {
            _statusWaitingForBandRegionSelection = true;
            lblStatus.Text = "Select the White area in the source (for white balancing)";
            _waitingToLearnBand = BandColor.White;
        }

        private void btnSaveHistograms_Click(object sender, EventArgs e)
        {
            // Store color state information for all 4 colors
            BandColor[] _colors = { BandColor.Red, BandColor.Orange, BandColor.Green, BandColor.Blue };
            for (int i = 0; i < 4; i++)
            {
                switch (_colors[i])
                {
                    case BandColor.Blue:
                        _colorCalibrationInformaiton[i] = new ColorCalibrationState(_colors[i], _blueBandCbHist,
                            _blueBandCrHist, tkbBlueThreshold.Value, tkbBlueErosion.Value, tkbBlueDilation.Value);
                        break;
                    case BandColor.Green:
                        _colorCalibrationInformaiton[i] = new ColorCalibrationState(_colors[i], _greenBandCbHist,
                            _greenBandCrHist, tkbGreenThreshold.Value, tkbGreenErosion.Value, tkbGreenDilation.Value);
                        break;
                    case BandColor.Orange:
                        _colorCalibrationInformaiton[i] = new ColorCalibrationState(_colors[i], _orangeBandCbHist,
                            _orangeBandCrHist, tkbOrangeThreshold.Value, tkbOrangeErosion.Value, tkbOrangeDilation.Value);
                        break;
                    case BandColor.Red:
                        _colorCalibrationInformaiton[i] = new ColorCalibrationState(_colors[i], _redBandCbHist,
                            _redBandCrHist, tkbRedThreshold.Value, tkbRedErosion.Value, tkbRedDilation.Value);
                        break;
                    default:
                        break;
                }
